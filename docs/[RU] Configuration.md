...
3. [Дверь и доступ к ним](https://github.com/GeTtOo/SCP-Breach-CSGO/blob/main/docs/%5BRU%5D%20Configuration.md#doorsjson)
...
**Простой способ:**  
1. Укажите путь к модели в формате
...
| legs | Ноги |
| chevron | Шеврон |
| body | Броня, жилеты и т.п. |
| belt | Ремень |
| pl | Карман на левой ноге |
| pr | Карман на правой ноге |

Не обязательно строго следовать названию. К примеру, Вы можете смело записать в параметр «шеврона» рюкзак. Но крайне рекомендуем всё же придерживаться концепции!  
...
Рандомизация нескольких предметов указывается в формате `[{"Процент":"Предмет", "Процент":"Предмет"}]`. После появление игроку выпадет один из перечисленных предметов[^1].  
**Пример:**  
```json
...
Пример:  
```json
"spawnlist":[{"pos":[0,0,0],"ang":[0,0,0],"color":[0,0,0],"size":15}]
```
...
В данном файле настраиваются двери и доступ к ним. Блок начинается с ID двери или кнопки, который можно узнать при включенном параметре `debug` в [config.json](https://github.com/GeTtOo/SCP-Breach-CSGO/blob/main/addons/sourcemod/configs/scp/scp_site101/config.json).  
...
Формат строки: `{"radius":Радиус,"pos":[Позици],"ltag":"Имя строки локализации "}`  
...
[^9]: В первом случае игроку необходимо прожать картой. Во втором случае игроку необходимо только нажать кнопку, система сама произведет поиск в инвентаре. 
