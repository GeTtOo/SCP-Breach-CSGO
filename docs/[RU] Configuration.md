...
Некоторые параметры поддерживают языковую принадлежность игрока. За это отвечает параметр {lang}.
...
| showoverlaytime | Целое число | Время показа информационного оверлея игроку в начале раунда |
| psars | Целое число | Время от начала раунда, после которого новые игроки перестанут появляться |

### Звуки  
| Параметр      | Значение      | Подробно  |
| ------------- |:-------------:| ---------|
| menuselect | "Путь к файлу" | Звук выбора меню |
| idpadag | "Путь к файлу"  | Звук панели доступа - Проход разрешён |
| idpadad | "Путь к файлу" | Звук панели доступа - Проход запрещён |

### Альфа-боеголовка  
| Параметр      | Значение      | Подробно  |
| ------------- |:-------------:| ---------|
| time | Целое число | Время с момента активации до взрыва боеголовки |
| ast | Целое число | Время через которое запустится автоматический подрыв комплекса |
| autostart | true или false | Разрешить автозапуск боеголовки по истечению времени |
| killpos | Целое число | Координата высоты ниже которой все игроки будут убиты при взрыве |
| immunedoors | \[0, 0, 0\] | Двери, которые не будут открыты при активации боеголовки |

**Блок звука:**  
| Параметр      | Значение      | Подробно  |
| ------------- |:-------------:| ---------|
| start | "Путь к звуковому файлу" | Аудиофайл, который будет воспроизведен при активации боеголовки |
| stop | "Путь к звуковому файлу" | Аудиофайл, который будет воспроизведен при отмене |
...
### Комната администратора и настройка прав
В этом блоке настраиваются координаты комнаты администратора и права необходимые на определённые действия.
| Параметр      | Значение      | Подробно  |
| ------------- |:-------------:| ---------|
...
| Параметр      | Значение      | Подробно  |
| ------------- |:-------------:| ---------|
| SHOW_PLAYER_CLASS | "Флаг администратора" | Флаг необходимый для просмотра класса игроков |
| RESPAWN_PLAYER | "Флаг администратора" | Флаг необходимый для возрождения игрока |
| TELEPORT | "Флаг администратора" | Флаг необходимый для телепорта игрока |
| REINFORCE | "Флаг администратора" | Флаг необходимый для призыва подкрепления | 
| GIVE_PLAYER_ITEM | "Флаг администратора" | Флаг необходимый для выдачи предметов и оружия |
| IGNORE_DOOR_ACCESS | "Флаг администратора" | Флаг необходимый для игнорирования карт доступа |
| MOVE_TO_ADMIN_ZONE | "Флаг администратора" | Флаг необходимый для проведения беседы с игроком |
| ROUND_RESTART | "Флаг администратора" | Флаг необходимый для перезапуска раунда |
| DESTROY_SITE | "Флаг администратора" | Флаг необходимый для активации системы, уничтожения комплекса |
...
### \[Опционально\] Tutorial[^6]  
В этой секции настраиваются точки появления игроков в зоне обучения по их языковой принадлежности.
Код языка берётся из Sourcemod.
...
### \[Опционально\] Sapper[^6]  
В этой секции настраиваются параметры плагина Sapper.

| Параметр      | Значение      | Подробно  |
| ------------- |:-------------:| ---------|
| bcmultipler | Целое число | Множитель урона от взрывчатки |
| bccount | Целое число | Максимальное количество взрывчатки |

### \[Опционально\] Handcuffs[^6]  
В этой секции настраиваются наручники.

| Параметр      | Значение      | Подробно  |
| ------------- |:-------------:| ---------|
| ammocount | Целое число | Количество боеприпасов в Тазере |
| breaktime | Целое число | Время на освобождение из наручников |
| breakingcd | Целое число | Время задержки между попытками освобождения |
| breakchance | Целое число | Шанс выбраться из наручников [^1] |

**Блок звука:**  
В этой секции указывается массив звуков (выбирается случайно).
...
**Блок настройки команд:**
В этом блоке настраивается какую команду и какой класс получит игрок после эвакуации в наручниках.

| Параметр      | Значение      | Подробно  | 
| ------------- |:-------------:| ---------|
| trigger | Целое число | ID триггера выхода |
| team | "Команда" | Команда, которую получит игрок |
| class | "Класс" | Класс, который получит игрок |
...
### \[Опционально\] Voice[^6] 
В этой секции настраиваются настройки голосового чата.

| Параметр      | Значение      | Подробно  | 
| ------------- |:-------------:| ---------|
| wtid | Целое число | ID world text ent |
| transmissiontime | Целое число | Время вещания интеркома |
| cooldown | Целое число | Время перезапуска интеркома |
| buttonid | Целое число | ID кнопки интеркома |
| localdistance | Целое число | Дистанция на которой слышно игрока | 

**Блок звука:**  
| Параметр      | Значение      | Подробно  |
| ------------- |:-------------:| ---------|
| start | "Путь к звуковому файлу" | Аудиофайл, который будет воспроизведен при старте вещания |
| stop | "Путь к звуковому файлу" | Аудиофайл, который будет воспроизведен при завершении вещания |
...
| searchzone | \[\[Координаты\], \[Координаты\]\] | Два вектора из которых строится куб поиска предметов [^7] |
| distance | \[Координаты\] | Вектор смещения относительно первой камеры [^8] |  

**Блок звука:**  
В этой секции указывается массив звуков который будет воспроизведён при смерти игрока (выбирается случайно).
...
**Блок звука:**  
В этой секции указывается массив звуков который будет воспроизведён при ходьбе SCP-049 (выбирается случайно).
...
### \[Опционально\] SCP-096[^6]  
**Блок исключения:**  
В этой секции указываются двери, которые сможет выбивать при агрессии SCP-096 (работает по имени модели).
...
**Блок звука:**  
В этой секции указывается массив звуков который будет воспроизведён при выбивании двери (выбирается случайно).
...
### \[Опционально\] SCP-106[^6]  

| Параметр      | Значение      | Подробно  |
| ------------- |:-------------:| ---------|
| cell | \[\[Координаты\], \[Координаты\]\] | Координаты клетки SCP-106 |
| ecop | Целое число | Шанс выхода из карманного измерения |
| blockdoors | \[0, 0, 0\] | ID дверей через которые не сможет проходить SCP-106 | 

**Блок измерения:**  
В этой секции указывается точки выхода из карманного измерения.
...
**Блок звука:**  
В этой секции указывается массив звуков который будет воспроизведён при установке портала в измерение и звуки шагов (выбирается случайно).
...
### \[Опционально\] SCP-996[^6]  

| Параметр      | Значение      | Подробно  |
| ------------- |:-------------:| ---------|
| damage | Число с плавающей точкой | Урон, наносимый |
| abradius | Целое число | Радиус действия способности |
| abcd | Целое число | Время перезарядки способности |
| abcsr | Число с плавающей точкой | Множитель замедления скорости жертвы при активации способности |

**Блок звука:**  
В этой секции указывается массив звуков который будет издавать SCP-966 (состояние спокойствия и состояние охоты).
...
| blinktime | Целое число | Через сколько секунд жертва будет моргать |
| blinkrange | Целое число | Дальность телепорта при моргании жертвы |
| minhpscale | Целое число | Количество HP при котором множитель способностей перестанет увеличиваться |

**Блок звуков:**  
В этой секции указывается массив звуков который будет воспроизведён, когда убивает SCP-173 или его смерти (выбирается случайно).
...
| sound | Путь к звуку (или массив) | Параметр звука (опционально) |
...
Каждый уровень ставится отдельно. 
...
Само название локации хранится в файле перевода `translations/scpcore.regions.txt`. Здесь же Вы указываете только имя строки перевода!  
...
В отличии от других параметров, общая сумма процентов не должна быть равна 100. Если Вы хотите сделать 50 процентный шанс появления предметов, то делите эти 50 процентов между всеми предметами. Таким образом, сначала сработает шанс появления предмета, а затем какой именно предмет из списка появится.  
...
Также поддерживается рандомизация без указания процента (у каждой позиции будут равные шансы).

Пример рандомизации без указания процента:
```json
"navigator":[
	[
...
```json
"имя предмета": [
  "Процентное соотношение":["Получаемый предмет", "Шанс уничтожения от 0 до 100", "Шанс успешного улучшения от 0 до 100 (Eсли улучшение будет провалено, предмет останется таким же)"]
]
```

В случае отсутствия параметра «Процентное соотношение», соотношение делится на равные доли между всеми получаемыми предметами.  
...
Поддерживается 5 эффектов:
1. Metamorphose - Временное бессмертие и возможность открытия всех дверей, но смерть в течение минуты
2. Heal - Регенерация здоровья (работает только если установлен плагин SCP_Medicine)
3. Speed - Увеличение скорости
4. Butchering - Смерть от распиливания на части
5. Injure - Кровотечение
...
| Команда       | Описание |
| ------------- | ---------|
| game_ready    | Завершить обучение |
| trs           | Сбросить прохождение обучения | 
| spec		| Перейти в режим наблюдателя |
...
| ents getall               | Вывести список хранилища объектов |
| player getall             | Вывести список всех игроков |
| player {ID} inv getall    | Показать инвентарь конкретного игрока |
| player {ID} inv drop {ID} | Выбросить предмет из инвентаря игрока |
| gm round end              | Принудительно завершить раунд |
| gm round lock             | Заблокировать раунд |
| gm round unlock           | Разблокировать раунд |
| gm round status           | Статистика раунда |
| gm timers                 | Список активных таймеров |
| gm se                     | Вывести список активных статус эффектов |
...
| debug getground           | Показать тип поверхности под игроком |
